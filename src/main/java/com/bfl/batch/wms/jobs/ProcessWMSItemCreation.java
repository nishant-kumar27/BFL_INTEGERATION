...
@Component("ProcessWMSItemCreation")
public class ProcessWMSItemCreation extends AbstractJob {
	
	Logger logger = LoggerFactory.getLogger(ProcessWMSItemCreation.class);
	...
	public void run() {
		...
		if(job.isEnabled()) {
			logger.info("ProcessWMSItemCreation job running...");
			String itemCode = "";
			try {
				String goLiveDate = ConfigProperties.getInstance().getConfigValue("GO_LIVE_DATE");
				
				List<ItemMasterDTO> itemMasterData = foundationDataService.getItemMasterData(goLiveDate);
...
//					request.setToken(token);
				...
				logger.info("ProcessWMSItemCreation completed Successfully");
			} catch (Exception e) {
