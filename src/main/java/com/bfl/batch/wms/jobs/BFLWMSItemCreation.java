...
@Component("BFLWMSItemCreation")
public class BFLWMSItemCreation extends AbstractJob {
	
	Logger logger = LoggerFactory.getLogger(BFLWMSItemCreation.class);
	...
	public void run() {
		...
		if(job.isEnabled()) {
			logger.info("BFLWMSItemCreation job running...");
			String itemCode = "";
			try {
				String goLiveDate = ConfigProperties.getInstance().getConfigValue("GO_LIVE_DATE");
				List<ItemMasterDTO> itemMasterData = foundationDataService.getItemData(goLiveDate);
				if(null != itemMasterData && itemMasterData.size() > 0 ) {
					...
					for(ItemMasterDTO itemMaster : itemMasterData) {
						...
						ItemMasterReq buildItemMaster = itemMasterHelperImpl.buildItemMasterForProd(itemMaster);
				...
				logger.info("BFLWMSItemCreation completed Successfully");
			} catch (Exception e) {
				logger.error("Error occured while publishing the messages to RICS: " + ExceptionUtils.getStackTrace(e));
//				ItemMasterConfigDTO itemMasterConfigDTO = setItemMasterConfigERROR(e.getMessage(), "N", itemCode);
//				foundationDataService.setExportedItems(itemMasterConfigDTO);
